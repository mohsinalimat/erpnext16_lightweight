...
REM start.bat - Start the ERPNext container, then its web server.
REM Native cmd version of start.ps1 - not affected by PowerShell execution policy.
...
REM Wait until the container is actually running before exec-ing into it
set /a tries=0
:waitloop
